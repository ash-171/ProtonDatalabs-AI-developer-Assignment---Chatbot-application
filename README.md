...
1. Modified `/predict` endpoint to read the query and context to give meaningful answer to the user.
2. I have used pre-trained model `distilbert-base-uncased-distilled-squad` and tokenizer `distilbert-base-uncased` as it was the only model which worked well. 
3. Implement the storage and handling of the incoming files from the frontend using `MongoDB`.
4. While reading the pdf files, it required additional libraries, which I couldn't solve in the limited timeframe.

### Frontend

1. Added a pop up which notifies that the file has been uploaded properly.
2. Extended the app's functionality to accept `.txt`,`.docx` files in addition to `.csv` files.
3. Did styling using `Material UI`.
